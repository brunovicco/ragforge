"""Tests for the Gemini summarization adapter, using a fake genai.Client (no network)."""
...
def test_raises_when_no_api_key_is_available(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(GenerationError, match="no Gemini API key"):
        GeminiSummarizer("gemini-3.1-flash-lite")
...
def test_summarize_returns_stripped_response_text(monkeypatch: pytest.MonkeyPatch) -> None:
    """A successful call returns the model's text, stripped of surrounding whitespace."""
    _install_fake_client(monkeypatch, lambda contents: _FakeResponse("  a summary.  "))
    summarizer = GeminiSummarizer("gemini-3.1-flash-lite", api_key="key")

    summary = summarizer.summarize(["texto um", "texto dois"])

    assert summary == "a summary."


def test_summarize_joins_every_text_into_the_prompt(monkeypatch: pytest.MonkeyPatch) -> None:
    """Every input text reaches the real prompt sent to the API."""
    fake_client = _install_fake_client(monkeypatch, lambda contents: _FakeResponse("summary"))
    summarizer = GeminiSummarizer("gemini-3.1-flash-lite", api_key="key")

    summarizer.summarize(["MARCADOR_UM", "MARCADOR_DOIS"])

    prompt = fake_client.models.calls[0]["contents"]
    assert "MARCADOR_UM" in prompt
    assert "MARCADOR_DOIS" in prompt
...
def test_raises_when_client_creation_fails(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(GenerationError, match="failed to create Gemini client"):
        GeminiSummarizer("gemini-3.1-flash-lite", api_key="key")
