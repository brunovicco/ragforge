...
| Run evidence (manifests, events, per-question records) | Benchmark runner (ADR-0017) | Reproducibility and audit | Legitimate interest in scientific integrity; contains no personal data | `artifacts/runs/`, `experiments/` (public repo) | Indefinite (published evidence) | Remove run from `published-runs.json` + delete directories |
